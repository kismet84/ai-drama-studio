...
import { Sparkles, Save, ArrowRight, ArrowLeft, Trash2, Plus, Loader2, ChevronDown, ChevronRight, Lock, LockOpen } from "lucide-react"
...
export default function ScriptWizardPage() {
  ...
  const addEpsSlot = useCallback(async () => {
    update({ totalEpisodes: (data.totalEpisodes || 0) + 5 })
  ...
  // ========== 手动解锁 ==========
  const unlockChar = useCallback((id: string) => {
    updChar(id, { locked: false })
  }, [updChar])

  const unlockEp = useCallback((epNum: number) => {
    update({
      episodeOutlines: data.episodeOutlines.map(e =>
        e.episodeNum === epNum ? { ...e, locked: false } : e
      ),
    })
  }, [data.episodeOutlines, update])
      ...
      <main className="flex-1 overflow-y-auto p-6">
        <div className="max-w-3xl mx-auto space-y-6">
          ...
          {step === 4 && <div className="space-y-4"><div><h3 className="text-lg font-bold">Step 4: 人物属性卡</h3><p className="text-sm text-zinc-500 mt-1">每个角色6要素：身份、性格、隐藏秘密、核心目的、弱点、标志性动作。建议4-6个。</p></div>
            ...
            <div className="space-y-3">{data.characters.map((c) => <Card key={c.id} className={cn(c.locked && "opacity-70")}><CardHeader className="pb-2"><div className="flex items-center justify-between"><CardTitle className="text-sm flex items-center gap-1.5">{c.locked && <Lock className="h-3.5 w-3.5 text-amber-500 shrink-0 cursor-pointer hover:text-red-500" onClick={() => unlockChar(c.id)} />}{c.name || "未命名"}</CardTitle><Button variant="ghost" size="icon" onClick={() => c.locked ? unlockChar(c.id) : delChar(c.id)} title={c.locked ? "点击解锁" : "删除"}>{c.locked ? <LockOpen className="h-3.5 w-3.5 text-amber-500 hover:text-emerald-500" /> : <Trash2 className="h-3.5 w-3.5 text-red-500" />}</Button></div></CardHeader>
              <CardContent className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {c.locked && <div className="sm:col-span-2 flex items-center gap-1 text-[10px] text-amber-600 dark:text-amber-400 mb-1 cursor-pointer hover:text-amber-700" onClick={() => unlockChar(c.id)}>
                  <LockOpen className="h-3 w-3" /> 点击解锁此角色
                </div>}
            ...
            <div className="space-y-2">{(["major", "medium", "hook"] as const).map((t) => { const items = data.suspenseList.filter((s) => s.type === t); if (!items.length) return null; const ls: Record<string, string> = { major: "大悬念", medium: "中层悬念", hook: "小钩子" }; const cs: Record<string, string> = { major: "bg-red-100 text-red-700", medium: "bg-amber-100 text-amber-700", hook: "bg-blue-100 text-blue-700" }
              if (t === "hook") { const groupSize = 10; const groups: SuspenseItem[][] = []; for (let i = 0; i < items.length; i += groupSize) groups.push(items.slice(i, i + groupSize)); const savedHook = data.collapsedHookGroups; const hasSavedHook = savedHook && savedHook.length > 0
                return <div key={t}><h4 className="text-sm font-semibold mb-2">{ls[t]}（{items.length}个，每10集一组折叠）</h4>{groups.map((group, gi) => { const startEp = gi * groupSize + 1; const endEp = Math.min((gi + 1) * groupSize, items.length); const collapsed = hasSavedHook ? savedHook!.includes(gi) : gi > 0; const toggle = () => { const s = new Set(hasSavedHook ? savedHook! : []); if (collapsed) s.delete(gi); else s.add(gi); update({ collapsedHookGroups: Array.from(s) }) }
                  return <div key={gi} className="mb-1"><button type="button" className="flex items-center gap-1.5 text-xs font-medium text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300 mb-1.5 transition-colors" onClick={toggle}>{collapsed ? <ChevronRight className="h-3 w-3" /> : <ChevronDown className="h-3 w-3" />}第 {startEp}-{endEp} 集钩子（{group.length}个）</button>{!collapsed && group.map((item, itemIdx) => <Card key={item.id} className={cn("mb-2", item.locked && "opacity-70")}><CardContent className="pt-3 pb-3 flex gap-2 items-start"><Badge className={cn("text-xs shrink-0", cs[t])}>第{startEp + itemIdx}集</Badge>{item.locked && <Lock className="h-3 w-3 text-amber-500 shrink-0 cursor-pointer hover:text-red-500" onClick={() => unlockSuspense(item.id)} />}<Input value={item.description} onChange={(e) => updSusp(item.id, { description: e.target.value })} placeholder={item.locked ? "已锁定" : "悬念描述..."} disabled={item.locked} className="h-8 text-sm flex-1" /><Button variant="ghost" size="icon" onClick={() => item.locked ? unlockSuspense(item.id) : delSusp(item.id)} title={item.locked ? "点击解锁" : "删除"}>{item.locked ? <LockOpen className="h-3.5 w-3.5 text-amber-500 hover:text-emerald-500" /> : <Trash2 className="h-3.5 w-3.5 text-red-500" />}</Button></CardContent></Card>)}</div> })}
              </div> }
              if (t === "medium") { const collapsed = !!data.collapsedMediumSuspense
                return <div key={t}><button type="button" className="flex items-center gap-1.5 text-sm font-semibold mb-2 hover:text-zinc-700 dark:hover:text-zinc-300 transition-colors" onClick={() => update({ collapsedMediumSuspense: !collapsed })}>{collapsed ? <ChevronRight className="h-3 w-3" /> : <ChevronDown className="h-3 w-3" />}{ls[t]}（{items.length}个）</button>{!collapsed && items.map((item) => <Card key={item.id} className={cn("mb-2", item.locked && "opacity-70")}><CardContent className="pt-3 pb-3 flex gap-2 items-start"><Badge className={cn("text-xs shrink-0", cs[t])}>{ls[t]}</Badge>{item.locked && <Lock className="h-3 w-3 text-amber-500 shrink-0 cursor-pointer hover:text-red-500" onClick={() => unlockSuspense(item.id)} />}<Input value={item.description} onChange={(e) => updSusp(item.id, { description: e.target.value })} placeholder={item.locked ? "已锁定" : "悬念描述..."} disabled={item.locked} className="h-8 text-sm flex-1" /><Input type="number" value={item.revealEpisode || ""} onChange={(e) => updSusp(item.id, { revealEpisode: parseInt(e.target.value) || undefined })} placeholder="揭晓集(Step7后回填)" disabled={item.locked} className="h-8 text-sm w-20" /><Button variant="ghost" size="icon" onClick={() => item.locked ? unlockSuspense(item.id) : delSusp(item.id)} title={item.locked ? "点击解锁" : "删除"}>{item.locked ? <LockOpen className="h-3.5 w-3.5 text-amber-500 hover:text-emerald-500" /> : <Trash2 className="h-3.5 w-3.5 text-red-500" />}</Button></CardContent></Card>)}</div> }
              return <div key={t}><h4 className="text-sm font-semibold mb-2">{ls[t]}</h4>{items.map((item) => <Card key={item.id} className={cn("mb-2", item.locked && "opacity-70")}><CardContent className="pt-3 pb-3 flex gap-2 items-start"><Badge className={cn("text-xs shrink-0", cs[t])}>{ls[t]}</Badge>{item.locked && <Lock className="h-3 w-3 text-amber-500 shrink-0 cursor-pointer hover:text-red-500" onClick={() => unlockSuspense(item.id)} />}<Input value={item.description} onChange={(e) => updSusp(item.id, { description: e.target.value })} placeholder={item.locked ? "已锁定" : "悬念描述..."} disabled={item.locked} className="h-8 text-sm flex-1" /><Input type="number" value={item.revealEpisode || ""} onChange={(e) => updSusp(item.id, { revealEpisode: parseInt(e.target.value) || undefined })} placeholder="揭晓集(Step7后回填)" disabled={item.locked} className="h-8 text-sm w-20" /><Button variant="ghost" size="icon" onClick={() => item.locked ? unlockSuspense(item.id) : delSusp(item.id)} title={item.locked ? "点击解锁" : "删除"}>{item.locked ? <LockOpen className="h-3.5 w-3.5 text-amber-500 hover:text-emerald-500" /> : <Trash2 className="h-3.5 w-3.5 text-red-500" />}</Button></CardContent></Card>)}</div> })}</div>
          ...
          {step === 7 && <div className="space-y-4"><div><h3 className="text-lg font-bold">Step 7: 分集大纲</h3><p className="text-sm text-zinc-500 mt-1">每集：标题、概要、核心冲突、结尾钩子。</p></div>
            <div className="flex gap-2 items-center"><Button variant="outline" onClick={genEps} disabled={!!generating} className="gap-2">{generating ? <Loader2 className="h-4 w-4 animate-spin" /> : <Sparkles className="h-4 w-4" />}生成下5集</Button><Button variant="outline" size="sm" onClick={addEpsSlot} className="gap-1"><Plus className="h-3 w-3" />+5集</Button><Button variant="outline" size="sm" className="text-red-500 hover:text-red-600" onClick={() => { if (confirm("确定清除第6集及以后的所有分集大纲？此操作不可撤销。")) clearEpsFrom(6) }}><Trash2 className="h-3 w-3" /> 清除后续</Button><div className="flex items-center gap-1.5 ml-auto"><span className="text-xs text-zinc-500">总集数:</span><Input type="number" value={data.totalEpisodes || 12} onChange={(e) => update({ totalEpisodes: parseInt(e.target.value) || 12 })} className="h-8 w-16 text-sm" /></div></div>
                ...
                <tbody>
                  {(episodeScenes[data.activeSceneEpisode || 1] || []).map((s, i) => (
                    <tr key={`${s.sceneNum}-${i}`} className={cn("border-t border-zinc-100 dark:border-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-800/30", s.locked && "opacity-60")}>
                      <td className="px-2 py-1 text-zinc-400">{s.sceneNum}{s.locked ? <span className="cursor-pointer hover:text-red-500" onClick={() => unlockScene(data.activeSceneEpisode || 1, s.sceneNum)} title="点击解锁"> 🔒</span> : ""}</td>
                      ...
                      <td className="px-2 py-1 text-center"><Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => s.locked ? unlockScene(data.activeSceneEpisode || 1, s.sceneNum) : delSc(data.activeSceneEpisode || 1, s.sceneNum)} title={s.locked ? "点击解锁" : "删除"}>{s.locked ? <LockOpen className="h-3 w-3 text-amber-500 hover:text-emerald-500" /> : <Trash2 className="h-3 w-3 text-red-400" />}</Button></td>
                    </tr>
                  ))}
                </tbody>
              </table>
