...
2. 网络小说叙事风格：第三人称叙述 + 自然嵌入对话，不要用剧本格式
3. ⚠️ 铁律：每个 ===SCENE N=== 块的汉字数必须在 200-400 字之间。少于 200 字说明描写不足，超过 400 字说明拖沓冗余
4. 对话用中文引号"直接引用"，无需标注情绪标签
5. ⚠️ 段与段之间必须空一行（用空行分隔），每段2-4句话，禁止连续大段文字
6. 保持快节奏，每段推动剧情，避免拖沓的环境描写
7. 每个场景以 ===SCENE N=== 作为分隔标记
8. ⚠️ 铁律：场景编号 N 必须从 1 开始连续递增到最后一个场景号，禁止跳号、禁止合并、禁止漏号
